...
Three files combine to form the full system prompt, baked into the Worker at deploy time:
...
Auto-generated by the parser from the GitHub Pages HTML. This is the bot's primary knowledge source for professional content.
...
├── parser/
│   ├── parser.py                  # Orchestrator — reads HTML, writes supplementary-context.txt
...
### 7.2 The HTML Parser (parser/parser.py)

A local Python script using `BeautifulSoup` that reads your GitHub Pages HTML files and extracts meaningful body content, stripping out navigation, footers, scripts, and styling boilerplate. HTML extraction logic lives in `html_extractor.py`; `parser.py` is a thin orchestrator. Output is `supplementary-context.txt`.
...
2. Prepends the system prompt (baked-in context constants) to the request
...
The bot is considered ready when it correctly handles:
...
10. Rate limit hit → friendly fallback with contact links
